...
public class BaseApi implements Api {
  ...
  protected <T extends IResponse> Future<T> get(
      String part, Request request, Class<T> responseClass, Map<String, String> headers) {
    return executorServiceDecorator.get(
        apiContext.connector(), createRequestContext(part, request, responseClass, headers));
  }

  private <T extends IResponse> RequestContext createRequestContext(
      String part, Request request, Class<T> responseClass, Map<String, String> headers) {
    require("part", part);
    ...
    return new RequestContext(url, apiKey, request, responseClass, headers);
  }

  protected <T extends IResponse> Future<T> post(
      String part, Request request, Class<T> responseClass, Map<String, String> headers) {
    return executorServiceDecorator.post(
        apiContext.connector(), createRequestContext(part, request, responseClass, headers));
  }
}
