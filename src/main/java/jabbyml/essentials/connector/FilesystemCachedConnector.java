...
public class FilesystemCachedConnector extends StandardConnector {
    ...
    public static final Duration CACHE_TIMEOUT = Duration.ofMinutes(60);

    private final File dir;

    public FilesystemCachedConnector(String prefix) {
        dir = new File(System.getProperty("java.io.tmpdir") + "/" + prefix);
        if (!dir.mkdirs()) {
    ...
    protected String getInitialValue(String url, RequestContext requestContext) throws IOException {
        String json = null;
        String hash = String.valueOf(url.hashCode() + requestContext.request().hashCode());
        ...
        if (file.exists()) {
            ...
            if (makeRequest) {
                log.info("cached file {} with size {} at {} for {} was invalid at {}", file.getAbsolutePath(),
                        json != null ? json.length() : 0, new Date(validTo), url, now);
                json = null;
            } else {
